...
ShadowNotes is a Zero-Trust Investigator's Notebook — the first on-device AI notebook purpose-built for air-gapped intelligence work. Built with the spirit of **Atmanirbhar Bharat** (Self-Reliant India) and the **#India2047** vision, it combines on-device speech recognition with a streaming on-device LLM to capture voice and extract structured intelligence in real-time, entirely within the browser via WebAssembly.

As **Prof. Yoshua Bengio** (Turing Award 2018, Université de Montréal) emphasized at the AI Impact Summit, responsible AI requires privacy by design and data governance — ShadowNotes is the practical embodiment of this principle.
...
- **Streaming LLM extraction** via `TextGeneration.generateStream()` — the crown jewel. Tokens appear in real-time as the Gemma 3 1B Instruct model processes transcripts through llama.cpp WASM, with domain-specific system prompts that correct speech recognition errors
...
- **Model lifecycle management** via `ModelManager` + `EventBus` + `OPFSStorage` — OPFS caching (~810MB persisted across sessions), real-time download progress tracking, and model registration with the LlamaCPP framework
...
The app supports 4 professional domains (Security Audit, Legal Deposition, Medical Notes, Incident Report), each with domain-specific system prompts that include speech-to-text error correction (e.g., "sequel injection" -> SQL injection, "hay BS corpus" -> habeas corpus). A two-layer extraction pipeline (single LLM generation with unified parsing → keyword regex fallback) with embeddings-based semantic deduplication ensures reliable results at every capability level.
...
**Zero Cost Overhead:** After the one-time ~900MB model download (cached in OPFS via `ModelManager`), every extraction session costs nothing. No per-token API fees, no subscriptions. This makes ShadowNotes viable for resource-constrained organizations — small law firms, independent auditors, field medics.
...
ShadowNotes solves a problem no existing tool addresses: professionals handling classified, HIPAA-protected, DPDPA-regulated (India's Digital Personal Data Protection Act 2023), or legally privileged information need AI-powered intelligence extraction but cannot transmit their data to any external service. Cloud AI tools (ChatGPT, Otter.ai) are disqualified by data sovereignty requirements. Encrypted note apps (Standard Notes) lack AI capabilities. ShadowNotes is the first tool that combines streaming on-device AI extraction with zero-network operation.

The app uses all three RunAnywhere Web SDK packages with **3 selectable LLM models** (Gemma 3 1B, Qwen2.5 0.5B, SmolLM2 135M) running on-device via llama.cpp WASM, with streaming token output (`generateStream`), structured output via unified parsing, RAG context injection via embeddings, and a VoiceAgent for hands-free conversational interaction. A two-layer extraction pipeline (single LLM generation with unified parsing → keyword regex fallback) with semantic deduplication ensures reliable results. The full ONNX audio pipeline (AudioCapture -> VAD (Silero) -> STT (Whisper) -> TTS (Piper)) enables true air-gapped voice operation. 4 domain profiles (Security/Kavach, Legal/Nyaaya, Medical/Sanjeevani, Incident/Prahari) provide tailored extraction with speech error correction. **jsPDF** generates domain-specific professional PDFs with Hindi-named branding. An onboarding tutorial guides first-time users. Brute-force protection and complete security headers (HSTS, Referrer-Policy, Permissions-Policy, CSP) harden the application. All data is encrypted with AES-256-GCM and authenticated via WebAuthn biometrics. 231 tests ensure quality across 18 test files. **Electron 35** provides a cross-platform desktop version.

Target users include security auditors, attorneys, physicians, and incident responders — especially those operating in India's diverse environments where connectivity is unreliable and data sovereignty under DPDPA 2023 is non-negotiable. This is **Atmanirbhar Bharat** in practice: technology self-reliance for India's most sensitive professional domains.

> Full vision: [About the Hackathon & India 2047 Vision](vision-india2047.md)
